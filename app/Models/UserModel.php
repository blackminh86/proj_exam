...
class UserModel extends AdminModel
{
    ...
    public function listItems($params = null, $options = null)
    {
        ...
        if ($options['task'] == "admin-list-items") {
            $query = $this->select('id', 'username', 'email', 'fullname', 'avatar', 'status', 'level', 'created_at', 'created_by', 'updated_at', 'modified_by');
    ...
    public static function getLevelNameAttribute($value)
    {
        $result = self::withDepth()->find($value->keys());
        $depth = $result->depth;
        return str_repeat('---|', $depth - 1 * 1) . ' ' . $value->name;
    }
}
